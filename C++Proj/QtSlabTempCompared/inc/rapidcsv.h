...
namespace rapidcsv {
    ...
    public:
        ...
        void SetColumn(const size_t pColumnIdx, const std::vector<T> &pColumn) {
            ...
            for (auto itRow = pColumn.begin(); itRow != pColumn.end(); ++itRow) {
                ...
                mData.at(static_cast<size_t>(std::distance(pColumn.begin(), itRow) + mLabelParams.mColumnNameIdx +
                                             1)).at(
                        dataColumnIdx) = str;
            }
        }
        ...
        void InsertRow(const size_t pRowIdx, const std::vector<T> &pRow = std::vector<T>(),
            ...
            if (pRow.empty()) {
                ...
                for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol) {
                    std::string str;
                    converter.ToStr(*itCol, str);
                    row.at(static_cast<size_t>(std::distance(pRow.begin(), itCol) + mLabelParams.mRowNameIdx +
                                               1)) = str;
                }
            }
    ...
    private:
        ...
        size_t GetDataColumnCount() const {
            const size_t firstDataRow = static_cast<size_t>((mLabelParams.mColumnNameIdx >= 0)
                                                            ? mLabelParams.mColumnNameIdx : 0);
            return (mData.size() > firstDataRow) ? mData.at(firstDataRow).size() : 0;
        }

        inline size_t GetDataRowIndex(const size_t pRowIdx) const {
            const size_t firstDataRow = static_cast<size_t>((mLabelParams.mColumnNameIdx + 1 >= 0) ?
                                                            mLabelParams.mColumnNameIdx + 1 : 0);
            return pRowIdx + firstDataRow;
        }

        inline size_t GetDataColumnIndex(const size_t pColumnIdx) const {
            const size_t firstDataColumn = static_cast<size_t>((mLabelParams.mRowNameIdx + 1 >= 0) ?
                                                               mLabelParams.mRowNameIdx + 1 : 0);
