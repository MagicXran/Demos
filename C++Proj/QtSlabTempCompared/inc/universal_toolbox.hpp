...
namespace uni_box {
    ...
    namespace coding {
        ...
        inline std::string WideByte2Acsi(std::wstring &wstrcode) {
            ...
            int convresult = ::WideCharToMultiByte(CP_OEMCP, 0, wstrcode.c_str(), -1, &resultstring[0], asciisize, NULL,
                                                   NULL);
    ...
    namespace time_tools {
        ...
        inline std::string getCurrentTime(const std::string &format, bool ms_flag) {
            try {
                ...
                if (ms_flag) {
                    oss << std::put_time(tm, format.c_str()) << "." << std::setfill('0') << std::setw(3)
                        << millis % 1000;
                } else {
    ...
    namespace utils {
        ...
        split(const std::string &src, const std::string &delimiter/*, std::vector<std::string>& dst*/) {
            //dst.clear();
            std::regex ws_re(delimiter);
            std::vector<std::string> v(std::sregex_token_iterator(src.begin(), src.end(), ws_re, -1),
        ...
        */
        ...
        template<typename T>
        inline bool NumberNonEmptyCheck(T &t1, T min_value = static_cast<T>(0), T max_value = static_cast <T>(999999),
                                        bool overlap = true) {
            std::cout << (typeid(t1).name()) << ":" << t1 << std::endl;
            ...
            if ((std::strncmp("float", typeid(t1).name(), 5) == 0) ||
                (std::strncmp("double", typeid(t1).name(), 6) == 0)) {
